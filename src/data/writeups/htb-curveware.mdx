...
<Callout type="info">
  To decrypt any file, strip the first 64 bytes (signature) and next 16 bytes (IV), then decrypt the
  payload using AES-256-CBC with the key $d$.
</Callout>
...
<Callout type="info">
  **Note:** SageMath is not available on Ubuntu 22.04 LTS. You can install it by adding the EPEL
...
1. **Hidden Number Problem (HNP)**: D. Boneh and R. Venkatesan, _"Hardness of Computing the Most Significant Bits of Secret Keys in Diffie-Hellman and Related Schemes"_, CRYPTO '96.
2. **ECDSA Nonce Attacks**: N. Smart, _"The Exact Security of ECDSA of Small Nonces"_, 2001.
