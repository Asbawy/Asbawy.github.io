  ...
  steps={[
    {
      ...
      title: "Scanning & Fuzzing",
      desc: "Nmap exposes SSH and a custom web app on port 1337. Fuzzing with the hmr_ prefix leaks an open log that dumps a valid username.",
      tools: ["Nmap", "ffuf"],
    },
    {
      phase: "Foothold",
      title: "OTP Brute Force & Account Takeover",
      desc: "Bypassed the password reset rate-limit with X-Forwarded-For spoofing, brute-forced the 4-digit recovery code, and reset the password to grab the user flag.",
      technique: "OTP Brute Force",
      tools: ["Python", "Caido"],
    },
    {
      phase: "PrivEsc",
      title: "JWT Forgery to RCE",
      desc: "Forged an admin HS256 token using an exposed key, then abused execute_command.php for command execution to retrieve the root flag.",
      technique: "JWT Forgery",
      tools: ["Caido", "curl"],
    },
  ]}
/>
...
_Hammer_ is a medium-rated Linux machine that front-ends a small custom PHP web app on a non-standard HTTP port (**1337**). The application features a login portal with a "forgot password" flow. Our objective is to capture two flags:
...
The script successfully stops at **`1034`**. With the correct code accepted, we are redirected to the _Set New Password_ form.
