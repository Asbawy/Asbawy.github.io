...
<Callout type="warning">
  **Critical Vulnerability:** The password reset token is derived from `SHA-1(server_timestamp +
  username)`. Since the timestamp is predictable (obtained from the HTTP response `Date` header) and
